...
```{r}
MultiOmicsGraphPrediction::PlotLineGraph(modelResults = modelResults, sampSubset = c("UACC.62", "NCI.ADR.RES", "HL.60.TB.", "UACC.257"), weights = TRUE, stype = "drug5FU")
```
...
```{r}
prunedPredictors <- MultiOmicsGraphPrediction::PrunePredictors(compositeSubgraphs = pairsPredAll, IntLIMresults = myres, modelResults = modelResults, inputData = inputDataTrain, covar = covar, verbose = TRUE)
```
# Consolidate the Pruned Predictors.
```{r}
consolidatedPredictorsSecondLayer <- MultiOmicsGraphPrediction::ObtainCompositeModelsFirstLayer(pairsInEachPredictor = pairsPredAll, importantPairs = prunedPredictors)
for(pairs in consolidatedPredictorsSecondLayer$compositeModels){
  MultiOmicsGraphPrediction::PlotLineGraph(modelResults = modelResults, subset = pairs, sampSubset = "UACC.62", weights = TRUE, stype = "drug5FU")
}
```
# Prune again.
```{r}
prunedAgain <- MultiOmicsGraphPrediction::PrunePredictorsHiddenLayers(compositeSubgraphs = consolidatedPredictorsSecondLayer, covar = covar,
                                                                      previousModels = prunedPredictors, IntLIMresults = myres, modelResults = modelResults, inputData = inputDataTrain, verbose = TRUE)
for(pairs in prunedAgain){
  MultiOmicsGraphPrediction::PlotLineGraph(modelResults = modelResults, subset = pairs, sampSubset = "UACC.62", weights = TRUE, stype = "drug5FU")
}
```
# Consolidate again
```{r}
consolidatedPredictorsThirdLayer <- MultiOmicsGraphPrediction::ObtainCompositeModelsFirstLayer(pairsInEachPredictor = consolidatedPredictorsSecondLayer$expandedCompositeModels, importantPairs = prunedAgain)
for(pairs in consolidatedPredictorsThirdLayer$compositeModels){
  MultiOmicsGraphPrediction::PlotLineGraph(modelResults = modelResults, subset = pairs, sampSubset = "UACC.62", weights = TRUE, stype = "drug5FU")
}
```
