...
hist(importance$pathway)
hist(importance$reaction)
...
print(MultiOmicsGraphPrediction::CompositePrediction(modelResults = modelResults, pairs = propagatedGraph))
```

## T-Test
(1) Try z-scores instead, or something else. Here, large error values are being de-emphasized due to the denominator term in the t-score.
(2) The final prediction needs to also be a combination of the final subgraphs. We shouldn't be averaging them.
```{r}
pairsPredAll <- MultiOmicsGraphPrediction::ObtainSubgraphNeighborhoods(modelInput = modelResults@model.input, percentOverlapCutoff = 50)
propagatedGraph <- MultiOmicsGraphPrediction::DoSignificancePropagation(pairs = pairsPredAll, modelResults = modelResults,
...
# Learn Weights.
We need to input the type of pruning and the cutoffs as parameters. This will allow
us to pass those parameters to the significance propagation step. We also need to take
out any references to subgraph averaging that were included in the gradient or prediction steps.
```{r}
modelInput <- MultiOmicsGraphPrediction::FormatInput(predictionGraphs = projectedGraph, 
                                                     coregulationGraph = coreg.r2,
                                                     inputData = inputDataTrain, 
...
modelResults <- MultiOmicsGraphPrediction::InitializeGraphLearningModel(modelInput = modelInput, learningRate = 0.9)
#optimalModel <- MultiOmicsGraphPrediction::OptimizeImportanceCombo(modelResults = modelResults,pruningMethod = "odds.ratio", verbose = TRUE)
