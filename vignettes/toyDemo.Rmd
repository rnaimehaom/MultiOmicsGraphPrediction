...
When we run cross-validation, we obtain the same pair (M2, G4) for each of the cross-validations.
```{r}
for(sample in rownames(inputData@sampleMetaData)){
  ...
  myres.cv <- IntLIM::RunIntLim(inputData = data$training, stype="age",
                                 save.covar.pvals = TRUE, 
                                 outcome = 1, 
                                 independent.var.type = 2, 
                                 continuous = TRUE)
  myres.all.cv <- IntLIM::ProcessResults(inputResults = myres.cv, inputData = data$training, 
                                       pvalcutoff = 1, rsquaredCutoff = 0, interactionCoeffPercentile = 0)
  myres.r2.density.cv <- density(myres.all.cv$rsquared)
  peak.r2.cv <- myres.r2.density$x[which.max(myres.r2.density.cv$y)]
  if(peak.r2.cv > 1){peak.r2.cv <- 1}
  if(peak.r2.cv < 0){peak.r2.cv <- 0}
  myres.r2.cv <- IntLIM::ProcessResults(inputResults = myres.cv, inputData = data$training, 
                                       pvalcutoff = 1, rsquaredCutoff = peak.r2.cv)
  pred.cv <- MultiOmicsGraphPrediction::RunPairwisePrediction(inputResults = myres.r2.cv, 
                                                         inputData = data$training,
                                                         stype = "age",
                                                         independentVarType = 2,
                                                         outcomeType = 1)
  coreg.r2.cv <- MultiOmicsGraphPrediction::BuildCoRegulationGraph(myres.r2.cv)
  projectedGraph.cv <- MultiOmicsGraphPrediction::ProjectPredictionsOntoGraph(coRegulationGraph = coreg.r2.cv,
                                                                         predictions = pred.cv)
  
  # Compute meta features.
  metafeaturesTrain <- MultiOmicsGraphPrediction::GetMetaFeatures(predictions = pred.cv,
                                                                 stype = "age",
                                                                 inputData = data$training,
  ...
  modelInput.cv <- MultiOmicsGraphPrediction::FormatInput(predictionGraphs = projectedGraph.cv, 
                                                     coregulationGraph = coreg.r2,
                                                     inputData = data$training, 
                                                     stype.class = "numeric",
                                                     stype = "age",
                                                     edgeTypeList = c("shared.outcome.analyte", "shared.independent.analyte"),
                                                     metaFeatures = metafeaturesTrain,
                                                     modelProperties = myres.r2.cv,
                                                     outcome = 1,
                                                     independent.var.type = 2)
  ...
  # modelResults.cv <- MultiOmicsGraphPrediction::InitializeGraphLearningModel(modelInput = modelInput.cv, learningRate = 0.2)
  # pairsPredAll.cv <- MultiOmicsGraphPrediction::ObtainSubgraphNeighborhoods(modelInput = modelResults.cv@model.input, percentOverlapCutoff = 50)
  # 
  # propagatedGraph.cv <- MultiOmicsGraphPrediction::DoSignificancePropagation(pairs = pairsPredAll.cv, modelResults = modelResults.cv,
  #                                                verbose = TRUE, makePlots = FALSE, pruningMethod = "error.t.test", includeVarianceTest = TRUE)
  # print(propagatedGraph.cv)
  # print(MultiOmicsGraphPrediction::CompositePrediction(modelResults = modelResults.cv, pairs = propagatedGraph.cv[[1]]))
  # 
