...
importance <- MultiOmicsGraphPrediction::GetMetaFeatures(predictions = pred, 
                                                                 stype = "age",
                                                                 inputData = inputData, 
...
for(sample in rownames(inputData@sampleMetaData)){
  ...
  projectedGraph <- MultiOmicsGraphPrediction::ProjectPredictionsOntoGraph(coRegulationGraph = coreg.r2.cv,
                                                                         predictions = pred.cv)
  ...
  modelInput.cv <- MultiOmicsGraphPrediction::FormatInput(predictionGraphs = projectedGraph, 
                                                     coregulationGraph = coreg.r2,
                                                     inputData = data$training, 
                                                     stype.class = "numeric",
                                                     stype = "age",
  ...
  optimalModel <- MultiOmicsGraphPrediction::OptimizeMetaFeatureCombo(modelResults = modelResults.cv,
                                                                     pruningMethod = "error.t.test", includeVarianceTest = TRUE, verbose = TRUE)
  
  pred.cv.test <- MultiOmicsGraphPrediction::RunPairwisePredictionTest(inputResults = myres.r2.cv, 
  ...
  # predictions <- DoPrediction(inputData = inputDataTest, metafeatures = metafeaturesTest, model = optimalModel)
}
```